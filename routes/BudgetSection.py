...
def getSection():
    ...
    data = pd.read_excel(
        "tempFiles/DXone ERP May 2025_07.11.2025-1200.xlsx", sheet_name="KPI Sheet"
    )
    ...
    for row in data.to_dict(orient="records"):
        ...
        if row["This Month"] != "":
            ...
            footerModelObj = FooterModel(
                ComparisonValue=comparisonValue, ComparisonText=comparisontext
            )

            cardModelObj = CardDataModel(
                Title=title, Content=content, Footer=footerModelObj
            )
    ...
    table = TableModel(Title="Budget Section", Column=headerRow, Rows=Rows)

    cards_data = cards
    charts_data = []

    # Combine into SectionData
    section_data = SectionData(Charts=charts_data, Cards=cards_data, Tables=[table])
