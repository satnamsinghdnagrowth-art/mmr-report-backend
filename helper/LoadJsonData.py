...
def load_section_configs():
    ...
    if SECTION_CARD_CONFIGS is None:  # Only load the configuration if it's not already loaded
        with open("config/SectionComponents.json", "r") as file:
            SECTION_CARD_CONFIGS = json.load(file)


# Call this function at the start of your app to load the configuration
